...
from .system_config import NODE_MODULE_CONFIG, ModuleConfig
...
class ConfigManager:
    ...
    def load_custom_config(self):
        """Tải cấu hình tùy chỉnh từ file JSON."""
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    self.custom_config = json.load(f)
            except Exception as e:
                print(f"Lỗi tải cấu hình tùy chỉnh: {e}")
                self.custom_config = {}
    
    def save_custom_config(self):
        """Lưu cấu hình tùy chỉnh ra file JSON."""
        try:
