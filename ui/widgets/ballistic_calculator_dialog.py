...
class BallisticCalculatorWidget(QWidget):
    ...
    def create_left_panel(self):
        ...
        capsule = self.create_input_row(label_text, "0", "m/s")
        layout.addWidget(capsule)
        ...
        # Trọng lượng đạn
        self.ammo_weight_input = self.create_input_row("Trọng lượng đạn", "0", "")
        layout.addWidget(self.ammo_weight_input)

        # Góc tạ mục tiêu
        self.target_angle_input = self.create_input_row("Góc tạ mục tiêu", "0", "")
    ...
    def create_input_row(self, label_text, default_value, unit):
        """Tạo một widget input hình viên thuốc."""
    ...
    def on_reset_clicked(self):
        ...
        # Reset trọng lượng đạn và góc tạ về 0
        self.ammo_weight_input.setText("0")
    ...
    def connect_signals(self):
        ...
        self.ammo_weight_input.textChanged.connect(self.update_angle_display)
    ...
    def on_mode_changed(self, is_auto):
        ...
        self.ammo_weight_input.setEnabled(is_auto)
    ...
    def calculate_corrections(self):
        ...
        try:
            air_temp = float(self.air_temp_input.text() or 0)
            air_density = float(self.air_density_input.text() or 0)
            charge_temp = float(self.charge_temp_input.text() or 0)
            ...
            # Tính lượng sửa nhiệt độ không khí (giả định: 0.1 ly giác/°C)
            temp_correction_mils = (air_temp - std_temp) * 0.1

            # Tính lượng sửa áp suất (giả định: 0.05 ly giác/mmHg)
            pressure_correction_mils = (air_density - std_pressure) * 0.05

            # Tính lượng sửa nhiệt độ liều phóng (giả định: 0.08 ly giác/°C)
            charge_correction_mils = (charge_temp - std_charge_temp) * 0.08

            # Tổng lượng sửa góc tầm (ly giác) - chung cho cả trái và phải
            elevation_correction_mils = temp_correction_mils + pressure_correction_mils + charge_correction_mils
            
            # Chuyển đổi từ ly giác sang độ: 1 ly giác = 0.05625 độ
            elevation_correction = elevation_correction_mils * 0.05625

            # Lượng sửa góc hướng do gió
            # Gió dọc ảnh hưởng đến góc tầm, gió ngang ảnh hưởng đến góc hướng
            wind_cross_low = float(self.wind_dir_inputs[0].text() or 0)   # Gió ngang thấp
            wind_cross_high = float(self.wind_dir_inputs[1].text() or 0)  # Gió ngang cao
            
            # Lượng sửa góc hướng do gió (giả định: 0.1 ly giác per m/s gió ngang)
            wind_correction_mils = (wind_cross_low + wind_cross_high) / 2 * 0.1
            
            # Chuyển đổi từ ly giác sang độ
            wind_correction = wind_correction_mils * 0.05625
            
            # Giả định: pháo trái và phải có vị trí đối xứng nên lượng sửa như nhau
            # Nếu cần khác nhau, có thể điều chỉnh dựa trên vị trí cụ thể
            elev_left_corr = elevation_correction
            elev_right_corr = elevation_correction
            dir_left_corr = wind_correction
            dir_right_corr = wind_correction
