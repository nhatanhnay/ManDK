# -*- coding: utf-8 -*-
"""
CAN Sender - Jetson Right
==========================
Gửi lệnh phóng đạn xuống CAN bus.
Jetson Right nhận lệnh từ Jetson1 qua webhook, sau đó gửi xuống CAN.
"""
...
def get_can_bus():
    ...
    if _can_bus is None:
        try:
            ...
            logger.error(f"Jetson RIGHT: Lỗi kết nối CAN bus: {e}")
            raise
    return _can_bus
...
def send_launch_command(idx, positions):
    ...
    try:
        ...
        bus.send(msg)
        logger.info(f"Jetson RIGHT: Gửi lệnh phóng - idx={idx}, positions={positions}")
        ...
        if e.errno == 19:
            logger.error(f"Jetson RIGHT: Không tìm thấy thiết bị CAN '{CAN_CHANNEL}'")
        else:
            logger.error(f"Jetson RIGHT: Lỗi CAN OSError: {e}")
        return False
        
    except Exception as e:
        logger.error(f"Jetson RIGHT: Lỗi gửi lệnh phóng: {e}")
        return False
...
if __name__ == '__main__':
    logger.info("Testing CAN sender for Jetson RIGHT...")
