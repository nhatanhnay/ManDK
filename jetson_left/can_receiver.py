# -*- coding: utf-8 -*-
"""
CAN Receiver - Jetson Left
===========================
Nhận trạng thái đạn từ CAN bus và cập nhật vào biến local.
Jetson Left sẽ gửi trạng thái này về Jetson1 qua webhook.
"""
            ...
            if msg.arbitration_id == CAN_ID_AMMO_STATUS:
                try:
                    ...
                    if data[1] != SIDE_CODE_LEFT:
                        continue  # Bỏ qua nếu không phải pháo trái
                    ...
                    logger.info(f"Jetson LEFT: Nhận trạng thái đạn {ammo_count}/18 sẵn sàng")
                    
                except Exception as e:
                    logger.error(f"Jetson LEFT: Lỗi xử lý CAN AMMO_STATUS: {e}")
    
    except Exception as e:
        logger.error(f"Jetson LEFT: Lỗi CAN receiver: {e}")
    finally:
        try:
            bus.shutdown()
        except:
...
def start_can_receiver():
    ...
    thread.start()
    logger.info("Jetson LEFT: CAN receiver thread đã khởi động")
    return thread
...
if __name__ == '__main__':
    logger.info("Starting CAN receiver for Jetson LEFT...")
    start_can_receiver()
