# Jetson Left (Pháo Trái) - Documentation

## Mục đích

Jetson Left điều khiển phần cứng của pháo TRÁI và giao tiếp với Jetson1 qua webhook.
...
### Tạo service file: /etc/systemd/system/jetson-left.service

```ini
[Unit]
Description=Jetson Left Webhook Server
After=network.target

[Service]
Type=simple
User=na
WorkingDirectory=/home/na/Projects/ManDK/jetson_left
ExecStart=/usr/bin/python3 /home/na/Projects/ManDK/jetson_left/webhook_server.py
Restart=always
RestartSec=10
...
sudo systemctl enable jetson-left.service
sudo systemctl start jetson-left.service
```

### Kiểm tra status

```bash
sudo systemctl status jetson-left.service
```
...
- Xem log: `sudo journalctl -u jetson-left.service -f`
...
```bash
# Xem log realtime
sudo journalctl -u jetson-left.service -f

# Xem log 100 dòng gần nhất
sudo journalctl -u jetson-left.service -n 100
```
