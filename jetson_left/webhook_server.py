# -*- coding: utf-8 -*-
"""
Jetson Left Webhook Server
===========================
Server webhook chạy trên Jetson Left (Pháo trái) để nhận lệnh từ Jetson1.
Server này xử lý các lệnh phóng đạn và điều khiển góc pháo TRÁI.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/jetson_left_webhook.log'),
...
class HardwareController:
    """
    Interface để điều khiển phần cứng pháo TRÁI.
    ...
    def launch_ammunition(idx, flag1, flag2, flag3, positions):
        """
        Thực hiện lệnh phóng đạn cho pháo TRÁI - Gửi xuống CAN bus.
        ...
        """
        try:
            logger.info(f"[HARDWARE LEFT] Lệnh phóng đạn - idx={idx}, positions={positions}")
            logger.info(f"[HARDWARE LEFT] Flags: {flag1:08b} {flag2:08b} {flag3:08b}")
            
            # Gửi lệnh xuống CAN bus
            success = can_sender.send_launch_command(idx, positions)
            
            if success:
                logger.info(f"[HARDWARE LEFT] ✅ Đã gửi lệnh phóng xuống CAN bus")
            else:
                logger.error(f"[HARDWARE LEFT] ❌ Lỗi gửi lệnh phóng xuống CAN bus")
            
            return success
            
        except Exception as e:
            logger.error(f"[HARDWARE LEFT] Lỗi khi phóng đạn: {e}")
            return False
    
    @staticmethod
    def set_cannon_angle(angle, direction):
        """
        Điều khiển góc pháo TRÁI.
        ...
        """
        try:
            # TODO: Thay thế bằng code thực tế điều khiển servo/motor pháo trái
            angle_deg = angle / 10.0
            direction_deg = direction / 10.0
            logger.info(f"[HARDWARE LEFT] Điều khiển pháo TRÁI - Góc: {angle_deg:.1f}°, Hướng: {direction_deg:.1f}°")
            
            return True
            
        except Exception as e:
            logger.error(f"[HARDWARE LEFT] Lỗi khi điều khiển pháo: {e}")
            return False
...
def handle_angle():
    """
    Nhận lệnh điều khiển góc pháo TRÁI từ Jetson1.
    ...
    try:
        ...
        logger.info(f"[API] Nhận lệnh góc pháo TRÁI - Góc: {angle/10:.1f}°, Hướng: {direction/10:.1f}°")
        
        # Điều khiển pháo trái
...
def health_check():
    ...
    return jsonify({
        "status": "healthy",
        "service": "Jetson Left Webhook Server",
        "side": "left"
    }), 200
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("Jetson Left Webhook Server Starting...")
    logger.info("=" * 60)
